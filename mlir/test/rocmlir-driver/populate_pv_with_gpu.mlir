...
// GEMM-HEURISTIC-CHECK: rock.gemm {{.*}} = {{.*}} * {{.*}} features = mfma|dot|atomic_add|atomic_add_f16 storeMethod = {{.*}}perf_config = "v2:32,64,4,32,64,1,1,1,1"{{.*}} : memref<[[CDIMS:[x0-9]+]]xf32> = memref<[[ADIMS:[x0-9]+]]xf32> * memref<[[BDIMS:[x0-9]+]]xf32>
