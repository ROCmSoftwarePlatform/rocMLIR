...
// CHECK_SCALE-NEXT: %[[currentSeqLen:.*]] = arith.constant 33 : index

// CHECK_SCALE-NEXT: rock.attention
// CHECK_SCALE-NEXT: qk = %[[queries]] * %[[keys]]
// CHECK_SCALE-NEXT: currentSeqLen = (%[[currentSeqLen]] : index)
...
// CHECK_SCALE: %[[keysCollapsed:.*]] = tensor.collapse_shape %[[keysAdd]] {{.*}} : tensor<2x2x32x1024xf32> into tensor<4x32x1024xf32>
...
// CHECK_SCALE: %[[valuesCollapsed:.*]] = tensor.collapse_shape %[[valuesAdd]] {{.*}} : tensor<2x2x1024x32xf32> into tensor<4x1024x32xf32>
// CHECK_SCALE-DAG: %[[keysTensor:.*]] = tosa.slice %[[keysCollapsed]] {size = array<i64: 4, 32, 33>, start = array<i64: 0, 0, 0>} : (tensor<4x32x1024xf32>) -> tensor<4x32x33xf32>
// CHECK_SCALE-DAG: %[[valuesTensor:.*]] = tosa.slice %[[valuesCollapsed]] {size = array<i64: 4, 33, 32>, start = array<i64: 0, 0, 0>} : (tensor<4x1024x32xf32>) -> [[valuesShape:tensor<.*>]]
// CHECK_SCALE: %[[qkTensor:.*]] = tosa.matmul %[[queriesTensor:.*]], %[[keysTensor]] : ([[queriesShape:tensor<.*>]], [[keysShape:tensor<.*>]]) -> [[squareShape:tensor<.*>]]
// CHECK_SCALE-DAG: %[[scaleTensor:.*]] = tosa.slice %[[origScaleTensor:.*]] {size = array<i64: 4, 1024, 33>, start = array<i64: 0, 0, 0>} : (tensor<4x1024x1024xf32>) -> tensor<4x1024x33xf32>
...
// CHECK_NO_SCALE-NEXT: %[[currentSeqLen:.*]] = arith.constant 33 : index

// CHECK_NO_SCALE-NEXT: rock.attention
// CHECK_NO_SCALE-NEXT: qk = %[[queries]] * %[[keys]]
// CHECK_NO_SCALE-NEXT: currentSeqLen = (%[[currentSeqLen]] : index)
...
// CHECK_NO_SCALE: %[[keysCollapsed:.*]] = tensor.collapse_shape %[[keysAdd]] {{.*}} : tensor<2x2x32x1024xf32> into tensor<4x32x1024xf32>
...
// CHECK_NO_SCALE: %[[valuesCollapsed:.*]] = tensor.collapse_shape %[[valuesAdd]] {{.*}} : tensor<2x2x1024x32xf32> into tensor<4x1024x32xf32>
// CHECK_NO_SCALE-DAG: %[[keysTensor:.*]] = tosa.slice %[[keysCollapsed]] {size = array<i64: 4, 32, 33>, start = array<i64: 0, 0, 0>} : (tensor<4x32x1024xf32>) -> tensor<4x32x33xf32>
// CHECK_NO_SCALE-DAG: %[[valuesTensor:.*]] = tosa.slice %[[valuesCollapsed]] {size = array<i64: 4, 33, 32>, start = array<i64: 0, 0, 0>} : (tensor<4x1024x32xf32>) -> [[valuesShape:tensor<.*>]]
