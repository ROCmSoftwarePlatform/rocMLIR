...
struct GreaterOrEqualConverter final : public OpConversionPattern<migraphx::GreaterOrEqual> {
...
LogicalResult
GreaterOrEqualConverter::matchAndRewrite(migraphx::GreaterOrEqual op, OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
  Value inA = adaptor.getInA();
  Value inB = adaptor.getInB();
  
  // Create a new tensor type with I1 element type
  auto newType = RankedTensorType::get(op.getType().getShape(), rewriter.getI1Type());
  auto goe = rewriter.createOrFold<tosa::GreaterEqualOp>(op->getLoc(), newType, inA, inB);
...
void migraphx::populateMIGraphXToTosaConversionPatterns(
  ...
  patterns.add<ConvConverter<ConvolutionOp>, ConvConverter<QuantConvolutionOp>,
               ...
               WhereConverter, GreaterOrEqualConverter>(typeConverter, patterns.getContext());
}
